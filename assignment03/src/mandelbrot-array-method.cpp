...
Benedict Hofmockel
4/20/2022
I started this file after reading the later lecture slides (telling me how to best design this) way too late into the project; this file was an effort ->
to make the program as recommended by those slides, but I had to cut my losses timewise in order to work on other assignments... :(
*/
...
the fact that the width is always 1500 pixels.*/
	...
	*/
	...
	myThreads[0] = std::thread(colorThread, red, green, blue, 0, (height/4)*1, 0, width, xStep, yStep, x1, y1);
	//Light Grey
	myThreads[1] = std::thread(colorThread, red, green, blue, (height/4)*1+1, (height/4)*2, 0, width, xStep, yStep, 1, 1);
	//Dark Grey
	myThreads[2] = std::thread(colorThread, red, green, blue, (height/4)*2+1, (height/4)*3, 0, width, xStep, yStep, 1, 1);
	//White
	myThreads[3] = std::thread(colorThread, red, green, blue, (height/4)*3+1, height, 0, width, xStep, yStep, 1, 1);
...
{
	long double xVal = xStart;
	long double yVal = yStart;
	int value = 0;
